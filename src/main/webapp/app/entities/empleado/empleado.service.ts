...
export class EmpleadoService {
    ...
    private convertDateFromClient(empleado: IEmpleado): IEmpleado {
        const copy: IEmpleado = Object.assign({}, empleado, {
            nacimiento: empleado.nacimiento != null && empleado.nacimiento.isValid() ? empleado.nacimiento.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((empleado: IEmpleado) => {
            empleado.nacimiento = empleado.nacimiento != null ? moment(empleado.nacimiento) : null;
        });
        return res;
    }
}
